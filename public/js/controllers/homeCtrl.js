...
/** ################################################################################## */    
    ...
    $scope.getCardData = function () {
        ...
        $http.get(`${CONFIG.baseUrl}/dashboard/card-data/${year}`)
        .then(function(res) {
            console.log(res);
    ...
    $scope.getSumMonthData = function () {
        ...
        ReportService.getSeriesData('/dashboard/sum-month-chart/', year)
        .then(function(res) {
            console.log(res);
            var debtSeries = [];
            var paidSeries = [];
            ...
            $scope.barOptions = ReportService.initBarChart("barContainer1", "รายงานยอดหนี้ ปีงบ " + (parseInt(year) + 543), categories, 'จำนวน');
    ...
    $scope.getSumYearData = function () {       
        ...
        .then(function(res) {
            ...
            $scope.barOptions = ReportService.initBarChart("barContainer2", "รายงานยอดหนี้ 3 ปีย้อนหลัง", categories, 'จำนวน');
            $scope.barOptions.series.push({
                name: 'หนี้คงเหลือ',
                data: debtSeries
            }, {
